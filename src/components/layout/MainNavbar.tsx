    ...
    >
      <nav className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          {/* Main Links */}
          <div className="flex items-center gap-6">
            <MainLink href="/cart" active={pathname === '/cart'}>
              Cart
            </MainLink>
            <MainLink href="/login" active={pathname === '/login'}>
              Sign In
            </MainLink>
          </div>
        </div>
...
function MainLink({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return (
    <Link
      href={href}
